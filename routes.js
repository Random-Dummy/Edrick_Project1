...
// Lyrics CRUD
// create Lyrics
router.post('/api/lyrics/add', async (req, res) => {
    try {
...
router.get('/api/lyrics/:id', async (req, res) => {
    try {
        let result = await db.getLyrics(req.params.id);
        res.status(200).json(result);
    } catch (e) {
        res.status(500).json({ message: e.message });
    }
...
router.put('/api/lyrics/:id', async (req, res) => {
    try {
        ...
        const result = await db.updateLyrics(lyrics._id, lyricsText);
        res.status(200).json({ message: result });
    } catch (e) {
        res.status(500).json({ message: e.message });
    }
...
router.delete('/api/lyrics/:id', async (req, res) => {
    try {
        const songID = req.params.id;
        // Find the lyrics document by song ID
        const existingLyrics = await db.getLyrics(songID);
        if (!existingLyrics) {
            return res.status(404).json({ message: "Lyrics not found for this song" });
        }
        const result = await db.deleteLyrics(existingLyrics._id);
        res.status(200).json({ message: result });
    } catch (e) {
        res.status(500).json({ message: e.message });
    }
